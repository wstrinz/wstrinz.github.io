...
Elm.RobosolverClient = Elm.RobosolverClient || {};
Elm.RobosolverClient.make = function (_elm) {
   "use strict";
   _elm.RobosolverClient = _elm.RobosolverClient || {};
   if (_elm.RobosolverClient.values)
   return _elm.RobosolverClient.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var cellDesc = function (cell) {
      return A2($Basics._op["++"],
      cell.name,
      A2($Basics._op["++"],"-",cell.note));
   };
   var cellBgStyleList = F2(function (model,cell) {
      var isActive = A2($Set.member,
   ...
   var cellWallStyleList = function (cell) {
      return $Set.toList(A2($Set.map,
      function (w) {
         return {ctor: "_Tuple2"
                ,_0: A2($Basics._op["++"],"border-",w)
                ,_1: "solid 1px black"};
      },
      cell.walls));
   };
   var cellStyle = F2(function (model,cell) {
      return $List.concat(_U.list([cellWallStyleList(cell)
                                  ,A2(cellBgStyleList,model,cell)]));
   });
   var disabledWallButtons = _U.list([A2($Html.p,
                                     _U.list([]),
                                     _U.list([$Html.text("Toggle Walls")]))
                                     ,A2($Html.button,_U.list([]),_U.list([$Html.text("Left")]))
                                     ,A2($Html.button,_U.list([]),_U.list([$Html.text("Right")]))
                                     ,A2($Html.button,_U.list([]),_U.list([$Html.text("Top")]))
                                     ,A2($Html.button,_U.list([]),_U.list([$Html.text("Bottom")]))]);
   var findCell = F2(function (coords,model) {
      var found = $List.head(A2($List.filter,
      function (cell) {
         return _U.eq(coords,{ctor: "_Tuple2",_0: cell.x,_1: cell.y});
      },
      $List.concat(model.board.rows)));
      return found;
   });
   var noSelectStyle = _U.list([{ctor: "_Tuple2"
   ...
   var inActiveCells = F2(function (cell,model) {
      return A2($Set.member,
      {ctor: "_Tuple2",_0: cell.x,_1: cell.y},
      model.activeCells);
   });
   var coordsMatch = F2(function (a,b) {
      return _U.eq(a.x,b.x) && _U.eq(a.y,b.y);
   });
   var updateWallsIfCellSingular = F3(function (model,
   wall,
   currentCell) {
      var isMatch = A2(inActiveCells,currentCell,model);
      var _p1 = isMatch;
      if (_p1 === true) {
            return A2($Set.member,
            wall,
            currentCell.walls) ? _U.update(currentCell,
            {walls: A2($Set.remove,
            wall,
            currentCell.walls)}) : _U.update(currentCell,
            {walls: A2($Set.insert,wall,currentCell.walls)});
         } else {
            return currentCell;
         }
   });
   var updateWallsIfCell = F3(function (model,wall,currentRow) {
      return A2($List.map,
      A2(updateWallsIfCellSingular,model,wall),
      currentRow);
   });
   var updateIfIsCell = F3(function (targetCell,
   newNote,
   currentRow) {
      return A2($List.map,
      function (cell) {
         return _U.eq(cell.x,targetCell.x) && _U.eq(cell.y,
         targetCell.y) ? _U.update(cell,{note: newNote}) : cell;
      },
      currentRow);
   });
   var update = F2(function (action,model) {
      var rows = model.board.rows;
      var board = model.board;
      var _p2 = action;
      switch (_p2.ctor)
      {case "NoOp": return model;
         case "ResetActiveCells": return _U.update(model,
           {activeCells: $Set.empty});
         case "SetClicking": return _U.update(model,
           {isClicking: _p2._0});
         default: var _p5 = _p2._1;
           var _p3 = _p2._0;
           switch (_p3.ctor)
           {case "Nothin": return model;
              case "SetActive": var _p4 = model.isClicking;
                if (_p4 === false) {
                      return model;
                   } else {
                      return A2(inActiveCells,_p5,model) ? _U.update(model,
                      {activeCells: A2($Set.remove,
                      {ctor: "_Tuple2",_0: _p5.x,_1: _p5.y},
                      model.activeCells)}) : _U.update(model,
                      {activeCells: A2($Set.insert,
                      {ctor: "_Tuple2",_0: _p5.x,_1: _p5.y},
                      model.activeCells)});
                   }
              case "SetNote": return _U.update(model,
                {board: _U.update(board,
                {rows: A2($List.map,A2(updateIfIsCell,_p5,_p3._0),rows)})});
              default: return _U.update(model,
                {board: _U.update(board,
                {rows: A2($List.map,
                A2(updateWallsIfCell,model,_p3._0),
                rows)})});}}
   });
   var CellUpdate = F2(function (a,b) {
      return {ctor: "CellUpdate",_0: a,_1: b};
   });
   var SetClicking = function (a) {
      return {ctor: "SetClicking",_0: a};
   };
   var ResetActiveCells = {ctor: "ResetActiveCells"};
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var ToggleWall = function (a) {
      return {ctor: "ToggleWall",_0: a};
   };
   var realWallButtons = F2(function (address,cell) {
      return _U.list([A2($Html.p,
                     ...
                     address,
                     A2(CellUpdate,ToggleWall("left"),cell))]),
                     _U.list([$Html.text("Left")]))
                     ,A2($Html.button,
                     _U.list([A2($Html$Events.onClick,
                     address,
                     A2(CellUpdate,ToggleWall("right"),cell))]),
                     _U.list([$Html.text("Right")]))
                     ,A2($Html.button,
                     _U.list([A2($Html$Events.onClick,
                     address,
                     A2(CellUpdate,ToggleWall("top"),cell))]),
                     _U.list([$Html.text("Top")]))
                     ,A2($Html.button,
                     _U.list([A2($Html$Events.onClick,
                     address,
                     A2(CellUpdate,ToggleWall("bottom"),cell))]),
                     _U.list([$Html.text("Bottom")]))]);
   });
   var cellEditor = F2(function (address,model) {
      var firstOfSet = $List.head($Set.toList(model.activeCells));
      var anActiveCell = function () {
         var _p6 = firstOfSet;
         if (_p6.ctor === "Nothing") {
               return $Maybe.Nothing;
            } else {
               return A2(findCell,_p6._0,model);
            }
      }();
      var _p7 = anActiveCell;
      if (_p7.ctor === "Nothing") {
            return A2($Html.div,
            _U.list([]),
            _U.list([$Html.text("(not editing a cell)")
                    ,A2($Html.div,_U.list([]),disabledWallButtons)]));
         } else {
            return A2($Html.div,
            _U.list([]),
            _U.list([A2($Html.p,
                    ...
                    function (thecell) {
                       ...
                       "editing ",
                       $Basics.toString(thecell)));
                    },
                    $Set.toList(model.activeCells)))
                    ,A2($Html.div,
                    _U.list([]),
                    A2(realWallButtons,address,_p7._0))]));
         }
   });
   var SetNote = function (a) {
      return {ctor: "SetNote",_0: a};
   };
   var SetActive = {ctor: "SetActive"};
   var cellCol = F3(function (address,model,cell) {
      return A2($Html.td,
      _U.list([$Html$Attributes.style(A2(cellStyle,model,cell))
              ,A2($Html$Events.onMouseDown,address,SetClicking(true))
              ,A2($Html$Events.onMouseEnter,
              address,
              A2(CellUpdate,SetActive,cell))]),
      _U.list([A2($Html.p,
      _U.list([]),
      _U.list([$Html.text(cellDesc(cell))]))]));
   });
   var cellRow = F3(function (address,model,row) {
      return A2($Html.tr,
      _U.list([]),
      A2($List.map,A2(cellCol,address,model),row));
   });
   var cellsDiv = F2(function (address,model) {
      return A2($List.map,
      A2(cellRow,address,model),
      model.board.rows);
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([A2($Html$Events.onMouseUp,address,SetClicking(false))
              ,$Html$Attributes.style(noSelectStyle)]),
      _U.list([A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,ResetActiveCells)]),
              _U.list([$Html.text("Clear Selection")]))
              ,A2($Html.table,
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                       ,_0: "border"
                                                       ,_1: "solid 1px black"}]))]),
              A2(cellsDiv,address,model))
              ,A2(cellEditor,address,model)]));
   });
   var Nothin = {ctor: "Nothin"};
   var genWalls = false;
   var initCell = F2(function (y,x) {
      var walls = function () {
         var _p8 = genWalls;
         if (_p8 === true) {
               return _U.eq(A2($Basics._op["%"],x,3),
               0) ? $Set.singleton("left") : _U.eq(A2($Basics._op["%"],y,5),
               0) ? $Set.singleton("top") : $Set.empty;
            } else {
               return $Set.empty;
            }
      }();
      return {name: "c",x: x,y: y,note: "",walls: walls};
   });
   var initRow = F2(function (length,y) {
      return A2($List.map,initCell(y),_U.range(1,length));
   });
   var initializeRows = F2(function (x,y) {
      return A2($List.map,initRow(x),_U.range(1,y));
   });
   var initialY = 16;
   var initialX = 16;
   var initialBoard = {rows: A2(initializeRows,initialX,initialY)
                      ,maxx: initialX
                      ,maxy: initialY};
   var initialModel = {board: initialBoard
                      ,activeCells: $Set.empty
                      ,isClicking: false};
   ...
   var Cell = F5(function (a,b,c,d,e) {
      return {name: a,x: b,y: c,note: d,walls: e};
   });
   var Board = F3(function (a,b,c) {
      return {maxx: a,maxy: b,rows: c};
   });
   var Model = F3(function (a,b,c) {
      return {board: a,activeCells: b,isClicking: c};
   });
   return _elm.RobosolverClient.values = {_op: _op
                                         ,Model: Model
                                         ,Board: Board
                                         ,Cell: Cell
                                         ,initialX: initialX
                                         ...
                                         ,main: main
                                         ,update: update
                                         ,updateIfIsCell: updateIfIsCell
                                         ,updateWallsIfCell: updateWallsIfCell
                                         ,updateWallsIfCellSingular: updateWallsIfCellSingular
                                         ...
                                         ,realWallButtons: realWallButtons
                                         ,disabledWallButtons: disabledWallButtons
                                         ,cellsDiv: cellsDiv
                                         ,cellRow: cellRow
                                         ,cellCol: cellCol
                                         ,cellStyle: cellStyle
                                         ,cellWallStyleList: cellWallStyleList
                                         ,cellBgStyleList: cellBgStyleList
                                         ,cellDesc: cellDesc};
};
